...
{
    public class IndexingKeyWords
    {
        public List<Document> SearchText(string word, List<int>? ids)
        {
            // загружаем все ключевые слова из БД в список
            List<KeyWord>? keywords = new List<KeyWord>();
            List<Document> documents = new List<Document>();
            using (DocumentsDbContext db = new DocumentsDbContext())
            {
                documents = db.Documents.Include(p => p.TypeDoc).ToList();
                keywords = db.KeyWords.ToList();
            }

            // список ключевых слов с Id документа
            List<Tuple<string, int>> keywordAndIdDoc = new List<Tuple<string, int>>();
            foreach (var item in keywords)
            {
                keywordAndIdDoc.Add(new Tuple<string, int>(item.Word, item.Document.Id));
            }

            // список ключевых слов для сравнения                                  int - IdDoc
            List<Tuple<string, string, int>> list = new List<Tuple<string, string, int>>();
            foreach (var item in keywords)
            {
                list.Add(new Tuple<string, string, int>(item.Word, "", item.Document.Id));
            }

            SetData(list);
            var request = Search(word);

            List<Tuple<string, double, int>> listVerified = new List<Tuple<string, double, int>>();
            foreach (var item in request)
            {
                if (word.Length <= 10)
                {
                    if (item.Item3 <= 370)
                    {
                        listVerified.Add(new Tuple<string, double, int>(item.Item1, item.Item3, item.Item5));
                    }
                }
                else if (word.Length > 10 && word.Length <= 25)
                {
                    if (item.Item3 <= 1070)
                    {
                        listVerified.Add(new Tuple<string, double, int>(item.Item1, item.Item3, item.Item5));
            ...
            listVerified.Sort((x, y) => x.Item2.CompareTo(y.Item2));  // сортируем индексы по возрастанию


            //foreach (var item in listVerified)
            //{
            //    Console.WriteLine($"{item.Item1} {item.Item2}");
            //}

            List<int> idDocs = new List<int>();
            foreach (var item in listVerified)
            {
                idDocs.Add(item.Item3);
            }
            idDocs = idDocs.Distinct().ToList(); // список с idDoc найденых по ключ. словам

            //foreach (var item in idDocs)
            //{
            ...
            using (DocumentsDbContext db = new DocumentsDbContext())
            {
                allDocuments = db.Documents.Include(p => p.TypeDoc).ToList();
            }

            List<Document> itsDoc = new List<Document>();
            foreach (var itemId in idDocs)
            {
                foreach (var itemDoc in allDocuments)
                {
            ...
            List<Document> itsDocSelect = new List<Document>();
            foreach (var doc in itsDoc)
            {
                foreach (var id in ids)
                {
                    if (doc.TypeDoc.Id == id)
                    {
                        itsDocSelect.Add(doc);
                    }
                }
            }
        ...
        public List<CompanyDocument> SearchTextCD(string word)
        {
            // загружаем все ключевые слова из БД в список
            List<CompanyDocument> comDoc = new List<CompanyDocument>();
            using (CompanyDbContext db = new CompanyDbContext())
            {
                comDoc = db.Documents.ToList();
            }

            // список ключевых слов с Id документа
            List<Tuple<string, int>> keywordAndIdDoc = new List<Tuple<string, int>>();
            foreach (var item in comDoc)
            {
                keywordAndIdDoc.Add(new Tuple<string, int>(item.Name, item.Id));
            }
            ...
            foreach (var item in comDoc)
            {
                list.Add(new Tuple<string, string, int>(item.Name, "", item.Id));
            }
            ...
            List<CompanyDocument> allDocuments;
            using (CompanyDbContext db = new CompanyDbContext())
            {
                allDocuments = db.Documents.ToList();
            }

            List<CompanyDocument> itsDoc = new List<CompanyDocument>();
            foreach (var itemId in idDocs)
            {
                foreach (var itemDoc in allDocuments)
                {
        ...
        public void SetData(List<Tuple<string, string, int>> datas)
        {
            ...
            foreach (var data in datas)
            {
                ...
                languageSet.IdDoc = data.Item3;
                this.Samples.Add(languageSet);
            }
        }


        public List<Tuple<string, string, double, int, int>> Search(string targetStr)
        {
            List<KeyValuePair<char, int>> codeKeys = CodeKeysRus.Concat(CodeKeysEng).ToList();
            ...
            var result = new List<Tuple<string, string, double, int, int>>();
            foreach (LanguageSet sampl in Samples)
            {
                ...
                result.Add(new Tuple<string, string, double, int, int>(sampl.Rus.Origianl, sampl.Eng.Origianl, cost, languageType, sampl.IdDoc));
            }
            return result;
        }
